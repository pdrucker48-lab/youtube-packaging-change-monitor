...
import {
    fetchPublicVideoObservations,
    resolvePublicChannels,
} from './lib/youtube-public.js';
...
try {
    ...
    if (!Array.isArray(channels) || channels.length === 0) {
        await Actor.pushData({
            ...
            ready: false,
            missing: ['channels'],
            setup: 'Add at least one public YouTube channel URL, @handle, or channel ID, then run again. No API key is required.',
            documentation: 'https://github.com/pdrucker48-lab/youtube-packaging-change-monitor#quick-start',
            observationalNotCausal: true,
        });
        log.info('Configuration required: channels');
    } else {
        ...
        const dataMode = youtubeApiKey ? 'official-api' : 'public-feed';
        const output = [];
        const failures = [];
        let videosObserved = 0;
        let baselineVideos = 0;

        const uniqueChannelReferences = [...new Set(channels)];
        const resolvedChannels = youtubeApiKey
            ? await resolveChannels(uniqueChannelReferences, youtubeApiKey, timeoutMs)
            : await resolvePublicChannels(uniqueChannelReferences, timeoutMs);
        ...
        for (const channel of resolvedChannels) {
            ...
            try {
                ...
                let fetched;
                if (youtubeApiKey) {
                    videoIds = await listRecentVideoIds(
                        channel.uploadsPlaylistId,
                        recentVideosPerChannel,
                        youtubeApiKey,
                        timeoutMs,
                    );
                    fetched = await fetchVideoObservations(videoIds, youtubeApiKey, timeoutMs);
                } else {
                    fetched = await fetchPublicVideoObservations(channel, recentVideosPerChannel, timeoutMs);
                    videoIds = fetched.map((video) => video.videoId);
                }
                ...
                for (const observation of observations) {
                    ...
                    const current = {
                        ...
                        dataSource: observation.dataSource ?? dataMode,
                        observedAt,
        ...
        const summary = {
            ...
            publicFeedVideoLimitPerChannel: dataMode === 'public-feed' ? 15 : null,
        ...
        log.info(`Observed ${videosObserved} videos in ${dataMode} mode and emitted ${output.length} signals.`);
    }
} finally {
    await Actor.exit();
}
...
function validateInput(input) {
    if (input.youtubeApiKey != null && typeof input.youtubeApiKey !== 'string') {
        throw new Error('youtubeApiKey must be a string when provided');
    }
